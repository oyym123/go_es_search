...
func (c *PushController) Push(w http.ResponseWriter, r *http.Request) {
	...
	//写入mongo
	var pushMongo models.PushMongoModel

	pushMongo.Create(models.PushMessageModel{SenderId: pm.SenderId, SenderName: pm.SenderName, Title: pm.Title, Content: pm.Content,
		Options: pm.Options, MsgType: pm.MsgType, PrimaryKey: pm.PrimaryKey, ResType: pm.ResType, UserIds: pm.UserIds, CreateTime: time.Now().Format(config.TIMESTAMP_FORMAT)})

	var msgId int64 = 1
	var data = config.MessageData{SenderId: pm.SenderId, MsgTime: time.Now().Format(config.TIMESTAMP_FORMAT), SenderName: pm.SenderName, Title: pm.Title, Content: pm.Content,
		Options: pm.Options, MsgId: msgId, PrimaryKey: pm.PrimaryKey, ResType: pm.ResType, MsgType: pm.MsgType}
	...
	if pm.UserIds == "0" { //发全部
		...
		if pm.MsgType == 2 { //如果是必达消息并发模式写入数据库状态为待发送
			go func() {
				...
				var pushMsgModel models.PushMessageModel
				msgId := pushMsgModel.Create(models.PushMessageModel{SenderId: pm.SenderId, SenderName: pm.SenderName, Title: pm.Title, Content: pm.Content,
					Options: pm.Options, PrimaryKey: pm.PrimaryKey, ResType: pm.ResType, MsgType: pm.MsgType, UserIds: pm.UserIds})

				waitSendUserIds := utils.SliceDiff(userIds, sendUserIds)
				var pushMsgLogModel models.PushMessageLogModel
				pushMsgLogModel.CreateWaiteMessageLogs(waitSendUserIds, msgId, pm.MsgType, time.Now().Format(config.TIMESTAMP_FORMAT), pm.PrimaryKey, pm.ResType)
			}()
		}
	}
