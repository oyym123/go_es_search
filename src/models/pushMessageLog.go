...
func (PushMessageLogModel) TableName() string {
	return "dcm_push_message_log"
}

//写入待发送消息日志
func (PushMessageLogModel) CreateWaiteMessageLogs(waitUserIds []interface{}, msgId int64, msgType int,
	createTime string, primaryKey string, resType int) {
	if msgId == 0 {
		return
	}
	...
	sql := "INSERT INTO dcm_push_message_log (msg_id, msg_type, user_id, create_time, update_time,primary_key,res_type) VALUES "
	// 循环data数组,组合sql语句
	for key, userId := range waitUserIds {
		if len(waitUserIds)-1 == key {
			//最后一条数据 以分号结尾
			sql += fmt.Sprintf("(%d, %d, %d,'%s','%s','%s','%d');", msgId, msgType, userId, createTime, createTime, primaryKey, resType)
		} else {
			sql += fmt.Sprintf("(%d, %d, %d,'%s','%s','%s','%d'),", msgId, msgType, userId, createTime, createTime, primaryKey, resType)
		}
	}
...
func (pml PushMessageLogModel) GetMustReadMsgByUserId(userId int64, unixtime int64) []config.MessageData {
	...
	for _, row := range data {
		...
		mustReadData = append(mustReadData, config.MessageData{SenderId: pm.SenderId, SenderName: pm.SenderName, MsgTime: pm.CreateTime, Title: pm.Title,
			Content: pm.Content, Options: pm.Options, MsgId: pm.ID, PrimaryKey: pm.PrimaryKey, ResType: pm.ResType, MsgType: pm.MsgType, MsgLogId: row.ID})
	}
